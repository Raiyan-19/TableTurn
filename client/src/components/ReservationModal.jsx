        ...
        >
          ...
          {step === 1 && (
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
              <div className="space-y-4">
                ...
                <div>
                  <label className="text-xs font-bold uppercase tracking-wider text-amber-700 dark:text-gold-400">
                      ...
                      <button
                        ...
                        className={`p-3.5 rounded-2xl border text-left flex flex-col justify-between transition-all ${
                          seatingArea === opt.name
                            ? 'bg-amber-500/15 border-amber-500/60 text-slate-950 dark:bg-gold-500/20 dark:border-gold-500/60 dark:text-white shadow-sm'
                            : 'bg-slate-50 dark:bg-surface-200 border-slate-200 dark:border-white/10 text-slate-700 dark:text-slate-300 hover:border-amber-400 dark:hover:border-white/20'
                        }`}
                      >
                        <div>
                          <p className="text-xs font-bold text-slate-900 dark:text-white">{opt.name}</p>
                          <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">{opt.description || 'Standard dining'}</p>
                        </div>
                        {seatingArea === opt.name && (
                          <span className="mt-2 text-[10px] font-bold text-amber-700 dark:text-gold-400 flex items-center gap-1">
          ...
          {step === 2 && (
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
              <div className="space-y-4">
                    ...
                    {OCCASIONS.map((occ) => (
                      <button
                        ...
                        className={`px-3 py-2 rounded-xl text-xs font-semibold text-left transition-all ${
                          occasion === occ
                            ? 'bg-amber-500/15 text-amber-800 dark:bg-gold-500/20 dark:text-gold-300 border border-amber-500/50 dark:border-gold-500/50 font-bold'
                            : 'bg-slate-50 dark:bg-surface-200 border border-slate-200 dark:border-white/10 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/5'
                        }`}
                      >
                        {occ}
                      </button>
                ...
                <div>
                  <label className="text-xs font-bold uppercase tracking-wider text-amber-700 dark:text-gold-400 flex items-center justify-between">
                    <span>Special Notes & Dietary Requirements</span>
                    <span className="text-[10px] text-slate-500 dark:text-slate-400 font-normal">Max 500 chars</span>
                  </label>
                  <textarea
                    rows={3}
                    ...
                    className="mt-2 w-full p-3 rounded-2xl bg-slate-50 dark:bg-surface-200 border border-slate-200 dark:border-white/10 text-xs text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:border-gold-500/60 transition-all resize-none shadow-inner"
                  />
                </div>
              ...
              <div className="mt-6 pt-4 border-t border-slate-200 dark:border-white/10 flex items-center justify-between">
                <button
                  onClick={() => setStep(1)}
                  className="px-4 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-white/5 dark:hover:bg-white/10 text-xs font-bold text-slate-700 dark:text-slate-300 flex items-center gap-1.5 transition-colors"
                >
              ...
              <form onSubmit={handleConfirmReservation} className="space-y-4">
                ...
                <div>
                  ...
                  <div className="mt-1.5 relative">
                    ...
                    <input
                      ...
                      className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-slate-50 dark:bg-surface-200 border border-slate-200 dark:border-white/10 text-xs text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:border-gold-500/60 transition-all shadow-inner"
                    />
                  </div>
                </div>
                ...
                <div>
                  <label className="text-xs font-bold uppercase tracking-wider text-amber-700 dark:text-gold-400 flex items-center justify-between">
                    ...
                    <span className="text-[10px] text-emerald-600 dark:text-emerald-400 font-bold">SMS / WhatsApp pass enabled</span>
                  </label>
                  <div className="mt-1.5 relative">
                    ...
                    <input
                      ...
                      className={`w-full pl-10 pr-4 py-2.5 rounded-xl bg-slate-50 dark:bg-surface-200 border text-xs text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none transition-all shadow-inner ${
                        phoneError ? 'border-crimson-500' : 'border-slate-200 dark:border-white/10 focus:border-gold-500/60'
                      }`}
                    />
                  </div>
                  {phoneError ? (
                    <p className="text-[11px] text-rose-600 dark:text-crimson-400 mt-1 flex items-center gap-1">
          ...
          {step === 4 && confirmedBooking && (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
              <div className="p-6 rounded-3xl bg-slate-50 dark:bg-gradient-to-b dark:from-surface-100 dark:to-surface-200 border border-amber-300 dark:border-gold-500/30 text-center shadow-md">
                
                {/* Success Icon */}
                <div className="w-14 h-14 rounded-full bg-emerald-500/20 border border-emerald-500/40 text-emerald-600 dark:text-emerald-400 flex items-center justify-center mx-auto mb-3 shadow-glow-emerald">
                ...
                <p className="text-xs text-slate-600 dark:text-slate-300 mt-1">
                  Your table at <strong className="text-amber-700 dark:text-gold-400">{bookingRestaurant.name}</strong> is locked.
                </p>

                {/* Unique Booking Code Badge */}
                <div className="my-4 inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-white dark:bg-black/50 border border-amber-300 dark:border-gold-500/40 shadow-xs">
                  <span className="text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400 tracking-wider">Booking Ref:</span>
                  <span className="font-mono text-sm font-extrabold text-amber-600 dark:text-gold-400 tracking-widest">
                    {confirmedBooking.reservationCode}
                  </span>
                </div>

                {/* Digital QR Code Pass */}
                <div className="p-4 rounded-2xl bg-white w-fit mx-auto shadow-xl my-2 border-4 border-amber-400/50 dark:border-gold-500/30">
                  <QRCodeSVG
                ...
                <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-2 text-left p-3 rounded-2xl bg-white dark:bg-black/30 border border-slate-200 dark:border-white/5 text-xs shadow-xs">
                  <div>
                    <span className="text-[10px] text-slate-500 dark:text-slate-400 block">Date</span>
                    <span className="font-bold text-slate-900 dark:text-white">{confirmedBooking.date}</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-500 dark:text-slate-400 block">Time</span>
                    <span className="font-bold text-amber-700 dark:text-gold-300">{confirmedBooking.timeSlot}</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-500 dark:text-slate-400 block">Guests</span>
                    <span className="font-bold text-slate-900 dark:text-white">{confirmedBooking.partySize} Persons</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-500 dark:text-slate-400 block">Seating</span>
                    <span className="font-bold text-slate-900 dark:text-white truncate">{confirmedBooking.seatingArea}</span>
                  </div>
                </div>
                ...
                <div className="mt-5 flex flex-wrap items-center justify-center gap-2.5">
                  <button
                    ...
                    className="px-3.5 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-white/10 dark:hover:bg-white/15 text-xs font-bold text-slate-800 dark:text-white flex items-center gap-1.5 transition-colors border border-slate-200 dark:border-white/10"
                  >
                    <CalendarPlus className="w-3.5 h-3.5 text-amber-600 dark:text-gold-400" />
                    Add to Calendar (.ics)
                  </button>
